...
export function GraphToolbar({
  ...
  return (
    <div className="flex max-w-full items-center gap-0.5 overflow-x-auto rounded-lg border border-border bg-surface/90 p-1 backdrop-blur">
      ...
      <select className="sr-only" value={source} onChange={(event) => onSourceChange(event.target.value as GraphSourceMode)}><option value="imported" disabled={!importedAvailable}>Import</option><option value="neo4j">Neo4j</option></select>
