...
import { clearAccessToken, currentSession, getAccessToken, login as authenticate, type SessionUser } from '../services/authApi'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!(import.meta as any).env?.PROD || getAccessToken()) void refresh()
    else setLoading(false)
    window.addEventListener('iag:unauthorized', logout)
    return () => window.removeEventListener('iag:unauthorized', logout)
  }, [refresh, logout])
