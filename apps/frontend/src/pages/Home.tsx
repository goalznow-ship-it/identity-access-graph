...
function Content({revision,onRefresh,refreshing}:{revision:number;onRefresh:()=>void;refreshing:boolean}){
  ...
  useEffect(()=>{const failure=error||summaryError;if(DEMO_DATA_ENABLED&&source==='neo4j'&&failure&&loadSettings(localStorage).autoFallback){if(importedAvailable){setNotice(`Neo4j Live unavailable: ${failure}. Switched to imported session.`);setSource('imported')}else{setNotice(`Neo4j Live unavailable: ${failure}. Switched to Mock Enterprise.`);setSource('mock')}}},[source,error,summaryError,setSource,importedAvailable])
