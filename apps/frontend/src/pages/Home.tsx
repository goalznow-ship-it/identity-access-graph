...
import { isEmptyGraph } from '../services/graphDataSource'
...
function Content({revision,onRefresh,refreshing}:{revision:number;onRefresh:()=>void;refreshing:boolean}){
  ...
  const {data,loading,error,retry}=useGraphData(null,source)
  ...
  useEffect(()=>{const controller=new AbortController();setSummaryError(null);void getDashboardSummary({}, {signal:controller.signal,timeoutMs:loadSettings(localStorage).apiTimeoutMs}).then(setSummary).catch((nextError)=>setSummaryError((nextError as Error).message));return()=>controller.abort()},[source,revision])
  useEffect(()=>{const failure=error||summaryError;if(source==='neo4j'&&failure&&loadSettings(localStorage).autoFallback){setNotice(`Neo4j Live unavailable: ${failure}. Switched to the persisted graph.`);setSource('imported')}},[source,error,summaryError,setSource])
  if(loading||(!summary&&!summaryError))return <DashboardSkeleton/>
  if(error||!data||summaryError)return <DashboardEmpty onRetry={()=>{void retry();onRefresh()}}/>
  if(isEmptyGraph(data))return <DashboardEmpty empty onRetry={()=>{void retry();onRefresh()}}/>
  const dashboardSummary=riskSummary?({...summary,riskCounts:riskSummary.countsBySeverity}as DashboardSummary):summary
  return <>{notice&&<div className="mb-3 rounded border border-warning/40 bg-warning/10 px-3 py-2 text-xs text-warning">{notice}</div>}<DashboardHeader nodes={summary?Object.values(summary.nodeTypeCounts).reduce((a,b)=>a+b,0):data.nodes.length} onRefresh={onRefresh} refreshing={refreshing} source={source} onSourceChange={setSource} importedAvailable/><div className="mt-5"><DashboardGrid data={data} summary={dashboardSummary}/>{attackSummary&&<div className="mt-5"><AttackPathOverview summary={attackSummary}top={topPaths}/></div>}</div></>
}
