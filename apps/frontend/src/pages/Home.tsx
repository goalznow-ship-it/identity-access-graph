...
function Content({revision,onRefresh,refreshing}:{revision:number;onRefresh:()=>void;refreshing:boolean}){
  ...
  useEffect(()=>{const failure=error||summaryError;if(source==='neo4j'&&failure&&loadSettings(localStorage).autoFallback){if(importedAvailable){setNotice(`Neo4j Live unavailable: ${failure}. Switched to imported session.`);setSource('imported')}}},[source,error,summaryError,setSource,importedAvailable])
