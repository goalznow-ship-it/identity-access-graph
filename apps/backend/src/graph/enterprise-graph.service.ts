...
export class EnterpriseGraphService {
  ...
  listSnapshots() { return this.snapshots.find({ order: { updatedAt: 'DESC' } }).then((rows) => rows.map((row) => ({ id: row.id, updatedAt: row.updatedAt, versionId: (row.payload as any).versionId, sequence: (row.payload as any).sequence, name: (row.payload as any).name, nodeCount: (row.payload as any).nodes?.length ?? 0, relationshipCount: (row.payload as any).relationships?.length ?? 0 }))) }
