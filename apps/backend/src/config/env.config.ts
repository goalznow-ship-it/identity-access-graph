export default () => {
  ...
  if ((process.env.AUTH_ENABLED ?? 'false').toLowerCase() === 'true' && (!jwtSecret || jwtSecret.length < 32)) {
    throw new Error('JWT_SECRET is required and must be at least 32 characters when AUTH_ENABLED=true')
  }
  return {
