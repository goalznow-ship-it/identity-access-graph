...
const productionConfig = { get: (key: string) => key === 'risk.allowDemoData' ? false : undefined }

describe('risk graph source safety', () => {
  it('fails closed when production has neither Neo4j nor an imported graph', async () => {
    const source = new RiskGraphSourceService({} as any, disabledNeo4j as any, undefined, productionConfig as any)
  ...
  it('hydrates and serves the durable imported graph without Neo4j', async () => {
    ...
    const source = new RiskGraphSourceService({} as any, disabledNeo4j as any, store as any, productionConfig as any)
  ...
  it('uses Neo4j for automatic production analysis when enabled', async () => {
    ...
    const source = new RiskGraphSourceService(graph as any, { isEnabled: () => true } as any, undefined, productionConfig as any)
